...
CodeBehind="w_sheet_cmd_ptinvtslipout.aspx.cs" Inherits="Saving.Applications.cmd.w_sheet_cmd_ptinvtslipout" %>
        ...
        function Validate() {
            try {
                for (var i = 1; i <= objDwDetail.RowCount(); i++) {
                    ...
                    var lot_id = getObjString(objDwDetail, i, "lot_id");
                    ...
                    //check invt_id and lot_id duplicate?
                    for (var j = 1; j <= objDwDetail.RowCount(); j++) {
                        var invtck_id = getObjString(objDwDetail, j, "invt_id");
                        var lotck_id = getObjString(objDwDetail, j, "lot_id");
                        if (i != j) {
                            if (invt_id == invtck_id && lot_id == lotck_id) {
                                alert("เลือกรหัสวัสดุและ รหัส lot ซ้ำกัน " + invt_id + " : " + lot_id + "\n กรุณาเลือกใหม่");
                                return;
                            }
                        }
                    }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlace" runat="server">
    ...
    <br />
    <dw:WebDataWindowControl ID="DwDetail" runat="server" DataWindowObject="d_slipout_detail"
    LibraryList="~/DataWindow/Cmd/cmd_ptinvtslipout.pbl" Width="750px" ClientScriptable="True"
    AutoRestoreContext="False" AutoRestoreDataCache="True" AutoSaveDataCacheAfterRetrieve="True"
