...
<tr><td style="font-size:14px;"><b>[SMSAD01]รายงาน : SMS เรียงตามวันเวลา</b> ณ วันที่ <?=$_REQUEST["OPERATE_DATE"]?> ถึง  <?=$_REQUEST["OPERATE_DATE1"]?> วันที่พิมพ์ <?=$_REQUEST["PRINT_DATE"]?> 
	 <?php 
	   ...
	   $sql.=" order by s.SEND_PRODATE asc,s.SMS_TRANS_CODE asc ";	
	   if($_REQUEST["SQL_FLAG"]=="1")
	     echo "<hr/>".$sql."<hr/>";
	   $value=array('REF_NO','PK_ID','MEMBER_NO','TELEPHONE_NUMBER','MESSAGE_TEXT','CREATE_DATE','SEND_DATE','SMS_TRANS_CODE','POST_FLAG','SEND_STATUS_MSG','SEND_PRODATE_','SMS_TRANS_DESC','POST_FLAG_');
        ...
        <center>-</center>
         <table class=table11_1 width="100%">
			...
			for($n=0;$n<$Num_Rows1;$n++){
					...
					$POST_FLAG_[$n] = conv($list_info1[$n][$m++]);
					$m=0;
			...
			?>
              <tr>
                ...
                <td align="center" >
                  ...
                  <?=$POST_FLAG_[$n]>0?("สำเร็จ(".$POST_FLAG[$n].")"):($POST_FLAG[$n]==0?"รอ":"ไม่สำเร็จ")?>
                </td>
                <td align="center" >
                  <?=$SEND_STATUS_MSG[$n]?>
                </td>
