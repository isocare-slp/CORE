...
<tr><td style="font-size:14px;"><b>[SMSSD01]รายงานสรุป : SMS รายวัน </b> ณ วันที่ <?=$_REQUEST["OPERATE_DATE"]?> ถึง  <?=$_REQUEST["OPERATE_DATE1"]?> วันที่พิมพ์ <?=$_REQUEST["PRINT_DATE"]?> 
	 <?php 
	   ...
	   $sql.="select s.*,to_char(s.SEND_PRODATE,'yyyy-mm-dd') as SEND_PRODATE_, p.SMS_TRANS_CODE||'-'||p.SMS_TRANS_DESC as SMS_TRANS_DESC from  SMSTRANSACTION s , smspatternconfig p ";
	   ...
	   $sql.=" ) group by SEND_PRODATE_ , SMS_TRANS_DESC order by SMS_TRANS_DESC asc, SEND_PRODATE_ asc  ";	
